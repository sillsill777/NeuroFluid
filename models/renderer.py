...
class RenderNet(nn.Module):
    ...
    def forward(self, physical_particles, ro, rays, focal, c2w, use_disp=False, perturb=0, noise_std=0.,
        ...
        """
        physical_particles: N_particles, 3      [11532, 3]
        ro: 3, camera location   [3, ]
        rays: N_rays, 6                     [1024, 6]
        ray_particles: N_ray, N_samples, 3
        zvals: N_rays, N_samples
        """
        results = {}
        N_rays = rays.shape[0]  # 1024
        # ---------------
        # coarse render
        # ---------------
        # coarsely sample
        ...
        if self.N_importance > 0:
            ray_particles_1, z_values_1 = ImportanceSampling(z_values_0, weights_0, self.N_importance, rays[..., :3],
                                                             rays[..., 3:], det=(perturb == 0))
            # search
    ...
    def coarse_rendering(self, physical_particles, ro, rays, focal, c2w, use_disp=False, perturb=0, noise_std=0.,
                         white_background=True):
        """
        ...
        dists_0, indices_0, neighbors_0, radius_0 = self.search(ray_particles_0, physical_particles, self.fix_radius)
        # embedding attributes
        pos_like_feats_0, dirs_like_feats_0, num_nn_0 = self.embedding_local_geometry(dists_0, indices_0, neighbors_0,
                                                                                      radius_0, ray_particles_0, rays,
                                                                                      ro)
        input_feats_0 = torch.cat(pos_like_feats_0 + dirs_like_feats_0, dim=1)
        # predict rgbsigma
        rgbsigma_0 = self.nerf_coarse(input_feats_0)
    ...
    def fine_rendering(self, physical_particles, ro, rays, focal, c2w, use_disp=False, perturb=0, noise_std=0.,
                       white_background=True):
        results = {}
        # ---------------
        # coarse render
        ...
        # --------
        # only need sigma
        pos_like_feats, = self.embedding_local_geometry(dists_0, indices_0, neighbors_0, radius_0, ray_particles_0,
                                                        rays, ro, sigma_only=True)
